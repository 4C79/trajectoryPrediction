        ...
        while (True):
            ...
            try:
                ...
                if recompensa_ar[-1:] == np.mean(recompensa_ar[-2:]) and recompensa_ar[-1:] > -5:
                    ...
                    kf = cv2.KalmanFilter(transition_matrices=np.array([[1, 1], [0, 1]]),
                                      transition_covariance=.001 * np.eye(2))

                    kf2 = cv2.KalmanFilter(transition_matrices=np.array([[1, 1], [0, 1]]),
                                       transition_covariance=.001 * np.eye(2))

                    kf3 = cv2.KalmanFilter(transition_matrices=np.array([[1, 1], [0, 1]]),
                                       transition_covariance=.001 * np.eye(2))
